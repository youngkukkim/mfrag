...
AUX_TARGETS = ['qed', 'sa']
SUPPORTED_TARGETS = DOCKING_TARGETS + AUX_TARGETS
...
class MoleculeEnv(gym.Env):
    ...
    def init(self, vocab, target, reward_mode='composite'):
        '''
    ...
    def _safe_qed(self, mols):
        ...
        for mol in mols:
            if mol is None:
                ...
                try:
                    scores.append(float(reward_qed([mol])[0]))
                except Exception:
                    scores.append(0.0)
        return np.asarray(scores, dtype=np.float32)

    def _safe_sa(self, mols):
        scores = []
        for mol in mols:
            if mol is None:
                scores.append(0.0)
                ...
                try:
                    scores.append(float(reward_sa([mol])[0]))
                except Exception:
                    scores.append(0.0)
        return np.asarray(scores, dtype=np.float32)

    def _target_scores(self, smiles, mols=None):
        if self.target_kind == 'docking':
            return np.asarray(reward_vina(smiles, self.predictor), dtype=np.float32)
        if mols is None:
            mols = [Chem.MolFromSmiles(s) for s in smiles]
        if self.target_kind == 'qed':
            return self._safe_qed(mols)
        if self.target_kind == 'sa':
            return self._safe_sa(mols)
        raise ValueError(f'Unsupported target kind: {self.target_kind}')

    def _aggregate_external_reward(self, target_scores, rq, rs):
        target_scores = np.asarray(target_scores, dtype=np.float32)
        if self.target_kind == 'docking':
            return np.clip(target_scores, 0, 20) / 20.0
        return np.clip(target_scores, 0.0, 1.0)
    ...
    def reward_batch(self, smiles=None):
        smiles = self.smile_list if smiles is None else smiles
        return self._target_scores(smiles)
    
    def reward_batch_vqs(self, smiles=None):
        smiles = self.smile_list if smiles is None else smiles
        mols = [Chem.MolFromSmiles(s) for s in smiles]
        rt = self._target_scores(smiles, mols)
        rq = self._safe_qed(mols)
        rs = self._safe_sa(mols)
        return (rt, rq, rs), self._aggregate_external_reward(rt, rq, rs)

    def step(self, ac):
        info = {}
        self.mol_old = copy.deepcopy(self.mol)
        atom_count_before = self.mol_old.GetNumAtoms()
        att_count_before = len(get_att_points(self.mol_old))
        ...
        if get_att_points(self.mol) == [] or self.mol.GetNumAtoms() > 30:
            if get_att_points(self.mol) == []:
                terminal_triggers.append('pre_no_attachment')
            if self.mol.GetNumAtoms() > 30:
                terminal_triggers.append('pre_max_atoms')
            new = True
        else:
            self._add_motif(ac)
        ...
        if new:
            ...
            if get_att_points(self.mol) != []:
                ...
                stop = True
                result_type = 'success_terminal'
            else:
                stop = False
                result_type = 'failed_terminal_no_attachment_after_trigger'
            self.counter = 0
        else:
            reward = reward_step
        ...
        info['terminal_trigger'] = '|'.join(terminal_triggers) if terminal_triggers else ''
        info['atom_count_before'] = atom_count_before
        info['atom_count_after'] = self.mol.GetNumAtoms()
        info['att_count_before'] = att_count_before
        info['att_count_after'] = len(get_att_points(self.mol))
