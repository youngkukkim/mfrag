...
def get_atom_features(atom, stereo, features, explicit_H=False):
    ...
    atom_features = atom_features + one_of_k_encoding_unk(atom.GetImplicitValence(), [0, 1, 2, 3])
    atom_features = atom_features + one_of_k_encoding_unk(atom.GetNumRadicalElectrons(), [0, 1])
    atom_features = atom_features + one_of_k_encoding(atom.GetDegree(), [0, 1, 2, 3, 4, 5, 6])
    atom_features = atom_features + one_of_k_encoding_unk(atom.GetFormalCharge(), [-1, 0, 1])
    atom_features = atom_features + one_of_k_encoding_unk(atom.GetHybridization(), [
        Chem.rdchem.HybridizationType.S,
        Chem.rdchem.HybridizationType.SP, Chem.rdchem.HybridizationType.SP2,
        Chem.rdchem.HybridizationType.SP3, Chem.rdchem.HybridizationType.SP3D])
    atom_features = atom_features + [int(i) for i in list("{0:06b}".format(features))]

    if not explicit_H:
        atom_features = atom_features + one_of_k_encoding_unk(atom.GetTotalNumHs(), [0, 1, 2, 3, 4])

    try:
        atom_features = atom_features + one_of_k_encoding_unk(stereo, ['R', 'S'])
        atom_features = atom_features + [atom.HasProp('_ChiralityPossible')]
    except Exception as e:
        atom_features = atom_features + [False, False] + [atom.HasProp('_ChiralityPossible')]
...
def get_graph_from_frag(smiles, idx=0):
    ...
    return Data(x=atom_feats, edge_index=edge_index, edge_attr=edge_feats, idx=idx, smiles=Chem.MolToSmiles(Chem.MolFromSmiles(smiles), isomericSmiles=False))
