...
class DockingVina(object):
    ...
    def docking(self, receptor_file, ligand_mol_file, ligand_pdbqt_file, docking_pdbqt_file):
        ...
        run_line = run_line + ' --center_x %s --center_y %s --center_z %s' %(self.box_center)
        run_line = run_line + ' --size_x %s --size_y %s --size_z %s' %(self.box_size)
        run_line = run_line + ' --cpu %d' % (self.num_cpu_dock)
        run_line = run_line + ' --num_modes %d' % (self.num_modes)
        run_line = run_line + ' --exhaustiveness %d ' % (self.exhaustiveness)
        result = subprocess.check_output(run_line.split(),
                                         stderr=subprocess.STDOUT,
        ...
        for result_line in result_lines:
            ...
            affinity_list = affinity_list + [affinity]
